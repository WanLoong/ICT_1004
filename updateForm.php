...
//submit_form.php
if (isset($_POST['updateFrmSubmit']) && !empty($_POST['name']) && !empty($_POST['email']) && (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) && !empty($_POST['password']) && !empty($_POST['confPassword'])&& !empty($_POST['userType'])&& !empty($_POST['userId'])) {
    ...
    if ($conn->connect_error) {
        ...
        $sql = "UPDATE p5_6.user_gp SET username='$name', password='$password', email='$email', type='$type' WHERE userID='$userID'";
        $status = 'ok';
